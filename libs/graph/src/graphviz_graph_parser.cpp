...
/* Like YYERROR except do call bgl_undir_error.  This remains here temporarily
...
do								\
  if (bgl_undir_char == YYEMPTY && yylen == 1)				\
    {								\
      bgl_undir_char = (Token);						\
      bgl_undir_lval = (Value);						\
    ...
    { 								\
      bgl_undir_error ("syntax error: cannot back up");			\
      YYERROR;							\
    }								\
while (0)
...
/* YYLEX -- calling `bgl_undir_lex' with the right arguments.  */
...
yyerrlab:
  ...
  if (!yyerrstatus)
      ...
      if (yyn > YYFLAG && yyn < YYLAST)
	{
	  ...
	  yysize += yystrlen (yytname[YYTRANSLATE (bgl_undir_char)]);
	  yymsg = (char *) YYSTACK_ALLOC (yysize);
	  if (yymsg != 0)
	    {
	      char *yyp = yystpcpy (yymsg, "parse error, unexpected ");
	      yyp = yystpcpy (yyp, yytname[YYTRANSLATE (bgl_undir_char)]);
	      ...
	      if (yycount < 5)
		{
	      ...
	      bgl_undir_error (yymsg);
	      YYSTACK_FREE (yymsg);
	    }
	  else
	    bgl_undir_error ("parse error; also virtual memory exhausted");
	}
      else
